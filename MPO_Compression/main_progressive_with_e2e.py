#!/usr/bin/env python3
"""
MPO 究极两阶段压缩管线 (Final Bulletproof Edition)
包含：Teacher-Forcing 数据截获、原始PPL输出、逐层参数精确追踪、断点毒素安检仪！
"""
...
def main():
    ...
    print("="*70)
    print(" 🚀 究极两阶段压缩管线启动 (Teacher-Forcing 绝对纯净版)")
    ...
    # ---------------------------------------------------------
    # 阶段二：累进式局部微调 (🚀 引入 Teacher Forcing 神级防线)
    # ---------------------------------------------------------
    if start_layer < num_layers:
        ...
        for layer_idx in range(start_layer, num_layers):
            ...
            print(f"    ✂️ 【参数雷达】本层参数从 {orig_layer_params/1e6:.2f}M 降至 {new_layer_params/1e6:.2f}M")
            print(f"    ✂️ 【瘦身战报】成功切除 {reduced_params/1e6:.2f}M 冗余参数！(保留率: {new_layer_params/orig_layer_params*100:.1f}%)")

            cached_h_in, cached_t_out = [], []
            
            # 🚨 终极安全措施：我们只截获 Teacher 模型的特征！
